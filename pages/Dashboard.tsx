        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-3xl font-black text-slate-900 dark:text-white flex items-center gap-3 uppercase italic tracking-tighter">
              {title}
              <span className="text-xs px-3 py-1 bg-slate-200 dark:bg-slate-700 rounded-full font-bold text-slate-600 dark:text-slate-300 not-italic tracking-normal">{filteredMachines.length} assets</span>
            </h2>
            <p className="text-slate-500 dark:text-slate-400 font-bold mt-1 tracking-wide">{subtitle}</p>
          </div>
          <div className="relative">
            ...
            <input
              ...
              className="pl-12 pr-4 py-3 rounded-2xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none w-full md:w-64 transition-all focus:md:w-72 shadow-sm"
              value={filter}
    ...
    <div className="space-y-8 relative text-left">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-bold mt-1 tracking-wide">Overzicht van alle openstaande meldingen.</p>
        </div>

        {hasPermission(Permission.CREATE_TICKET) && (
          <button
            onClick={() => setShowTicketModal(true)}
            className="bg-red-600 hover:bg-red-700 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg shadow-red-500/30 flex items-center gap-3 transition-transform active:scale-95"
          >
      ...
      {showTicketModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-6 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] shadow-2xl w-full max-w-lg border border-slate-200 dark:border-slate-700">
            ...
            <form onSubmit={handleQuickTicketSubmit} className="space-y-6">
              <div>
                <label className="block text-[10px] uppercase tracking-widest font-black text-slate-500 mb-2">Selecteer Machine</label>
                <select
                  required
                  className="w-full p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold dark:text-white outline-none focus:ring-2 focus:ring-red-500"
                  value={qtMachineId}
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-widest font-black text-slate-500 mb-2">Titel van storing</label>
                <input
                  required
                  type="text"
                  className="w-full p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold text-lg dark:text-white outline-none focus:ring-2 focus:ring-red-500"
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-widest font-black text-slate-500 mb-2">Omschrijving</label>
                <textarea
                  className="w-full p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-medium dark:text-white outline-none focus:ring-2 focus:ring-red-500 min-h-[100px]"
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-widest font-black text-slate-500 mb-2">Urgentie & Impact</label>
                <div className="grid grid-cols-2 gap-4">
                  <button
                    type="button"
                    onClick={() => setQtImpact(TicketImpact.NORMAL)}
                    className={`p-4 rounded-2xl border-2 font-black uppercase text-xs tracking-widest transition-all shadow-sm ${qtImpact === TicketImpact.NORMAL ? 'bg-blue-50 border-blue-500 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 ring-2 ring-blue-500/20' : 'bg-white border-slate-200 dark:border-slate-700 dark:bg-slate-800 text-slate-400 hover:border-slate-300'}`}
                  >
                    <div className="flex items-center gap-2 justify-center mb-1"><CalendarClock size={20} /> Normaal</div>
                    <div className="text-[9px] opacity-70 tracking-normal mt-1">+ Kan wachten</div>
                  </button>
                  <button
                    type="button"
                    onClick={() => setQtImpact(TicketImpact.CRITICAL)}
                    className={`p-4 rounded-2xl border-2 font-black uppercase text-xs tracking-widest transition-all shadow-sm ${qtImpact === TicketImpact.CRITICAL ? 'bg-red-50 border-red-500 text-red-700 dark:bg-red-900/40 dark:text-red-300 ring-2 ring-red-500/20' : 'bg-white border-slate-200 dark:border-slate-700 dark:bg-slate-800 text-slate-400 hover:border-slate-300'}`}
                  >
                    <div className="flex items-center gap-2 justify-center mb-1"><Zap size={20} /> Kritiek</div>
                    <div className="text-[9px] opacity-70 tracking-normal mt-1">+ Meteen Checken</div>
                  </button>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                <button
                  type="submit"
                  className="px-10 py-4 bg-red-600 hover:bg-red-700 text-white font-black uppercase text-xs tracking-widest rounded-2xl shadow-lg shadow-red-500/30 transition-transform active:scale-95 flex items-center gap-3"
                >
                  <Siren size={16} /> Melding Versturen
                </button>
              </div>
            </form>
          </div>
