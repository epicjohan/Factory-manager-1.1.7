...
import { Wrench, Calendar, Clock, CheckCircle, Eye, Printer, Filter, Trash2 } from '../icons';
...
export const ToolPreparation: React.FC = () => {
    ...
    const [filterStatus, setFilterStatus] = useState<ToolRequestStatus | 'ALL' | 'HISTORY'>('ALL');
    ...
    const deleteRequest = async (id: string, name: string) => {
        const isConfirmed = await confirm({
            ...
            message: `Weet je zeker dat je de gereedschapsaanvraag voor "${name}" wilt verwijderen? Deze actie is definitief.`,
        ...
        if (isConfirmed) {
            try {
                await db.deleteToolPrepRequest(id);
                addNotification('SUCCESS', 'Oproep Verwijderd', 'De gereedschapslijst oproep is verwijderd.');
                ...
                addNotification('ERROR', 'Fout', 'Opslaan mislukt.');
            }
        }
    };
    ...
    // Split ready requests to handle history
    const readyRequests = requests.filter(r => r.status === ToolRequestStatus.READY);
    const topReadyIds = new Set(readyRequests.slice(0, 30).map(r => r.id));
    
    const filteredRequests = requests.filter(r => {
        if (filterStatus === 'ALL') return true;
        if (filterStatus === 'HISTORY') {
            return r.status === ToolRequestStatus.READY && !topReadyIds.has(r.id);
        }
        if (filterStatus === ToolRequestStatus.READY) {
            return r.status === ToolRequestStatus.READY && topReadyIds.has(r.id);
        }
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden min-h-[600px] flex flex-col relative">
                ...
                <div className="p-6 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/20 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex bg-white dark:bg-slate-800 p-1 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm self-start">
                        {(['ALL', ToolRequestStatus.PENDING, ToolRequestStatus.IN_PROGRESS, ToolRequestStatus.READY, 'HISTORY'] as const).map(status => (
                            <button
                                key={status}
                            ...
                            >
                                {status === 'ALL' ? 'Alles' : status === 'HISTORY' ? 'Historie' : getStatusLabel(status as ToolRequestStatus)}
                            </button>
                        ))}
                    </div>
                </div>
                            ...
                            return (
                                <div key={req.id} className="bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-[2rem] p-5 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 group">
                                    ...
                                    <div className="flex flex-col md:flex-row items-center gap-3 shrink-0 border-t md:border-t-0 md:border-l border-slate-100 dark:border-slate-700 pt-4 md:pt-0 md:pl-6">
                                        ...
                                        <button 
                                            onClick={() => deleteRequest(req.id, req.setupName)}
                                            className="w-full md:w-auto p-3 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-600 dark:hover:text-white rounded-2xl font-black uppercase text-xs tracking-widest transition-all"
