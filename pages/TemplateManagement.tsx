        ...
        <div className="max-w-7xl mx-auto pb-20 text-left h-[calc(100vh-100px)] flex flex-col">
            ...
            <div className="flex gap-8 flex-1 min-h-0">
                ...
                <div className="w-80 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden shrink-0">
                    <div className="p-4 border-b border-slate-100 dark:border-slate-700">
                        <button onClick={handleCreateNew} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20 transition-all">
                    ...
                    {isEditing ? (
                        <>
                            <div className="p-8 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
                                <div className="flex justify-between items-start mb-6">
                                    <div className="flex-1 max-w-2xl grid grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input 
                                                type="text" 
                                                className="w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 font-bold text-lg outline-none focus:ring-2 focus:ring-indigo-500"
                                                value={editData.name || ''}
                                        ...
                                        <div>
                                            ...
                                            <select 
                                                className="w-full p-3.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 font-bold outline-none focus:ring-2 focus:ring-indigo-500"
                                    ...
                                    <div className="flex gap-2">
                                        {editData.id && !editData.isDefault && (
                                            <button onClick={() => handleDeleteTemplate(editData.id!)} className="p-3 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all">
                                                <Trash2 size={20} />
                                            </button>
                                        )}
                                        <button onClick={handleSaveTemplate} className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center gap-2 shadow-lg">
                                ...
                                {!isProcessTemplate ? (
                                    <div className="flex gap-1 p-1 bg-slate-200 dark:bg-slate-700 rounded-xl w-fit animate-in fade-in duration-200">
                                        <button 
                                            onClick={() => setActiveTab('FIXTURE')}
                                            className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'FIXTURE' ? 'bg-white dark:bg-slate-800 text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                        >
                                            <Layers size={14} /> Opspanning Velden
                                        </button>
                                        <button 
                                            onClick={() => setActiveTab('TOOLS')}
                                            className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'TOOLS' ? 'bg-white dark:bg-slate-800 text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                        >
                                            <Wrench size={14} /> Gereedschap Velden
                                        </button>
                                    </div>
                                ) : (
                                    <div className="flex items-center gap-2 px-6 py-3 bg-blue-600/10 text-blue-600 rounded-xl border border-blue-500/20 w-fit animate-in slide-in-from-top-2 duration-300">
                                        <Box size={16} />
                            ...
                            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-slate-50/50 dark:bg-slate-900/20">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <button onClick={() => openFieldEditor()} className="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold shadow-sm hover:border-indigo-500 hover:text-indigo-600 transition-all flex items-center gap-2">
                                ...
                                <div className="space-y-3">
                                    {getActiveFields().map((field, idx) => (
                                        <div key={idx} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-4 group hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
                                            ...
                                            <div className={`p-3 rounded-lg ${field.type === 'header' ? 'bg-slate-100 text-slate-600' : 'bg-indigo-50 text-indigo-600'} dark:bg-slate-700 dark:text-slate-300`}>
                                                {(() => {
                                            ...
                                            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => openFieldEditor(idx)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"><Edit size={16}/></button>
                                                <button onClick={() => deleteField(idx)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16}/></button>
                                            </div>
                                        </div>
                                    ))}
            ...
            {showFieldModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in duration-200 border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                            <div>
                                ...
                                <input autoFocus type="text" className="w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-bold" value={fieldData.label} onChange={e => setFieldData({ ...fieldData, label: e.target.value })} placeholder="Bijv. Spandruk" />
                            </div>
                            
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Type Invoer</label>
                                    <select className="w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900" value={fieldData.type} onChange={e => setFieldData({ ...fieldData, type: e.target.value as SetupFieldType })}>
                                ...
                                <div>
                                    ...
                                    <input type="text" className="w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-mono text-sm" value={fieldData.key} onChange={e => setFieldData({ ...fieldData, key: e.target.value })} placeholder="auto_generated" />
                                </div>
                            </div>
                            ...
                            {fieldData.type !== 'header' && (
                                <div>
                                    ...
                                    <select 
                                        className="w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-bold"
                            ...
                            {fieldData.type === 'number' && (
                                <div>
                                    ...
                                    <input type="text" className="w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900" value={fieldData.unit || ''} onChange={e => setFieldData({ ...fieldData, unit: e.target.value })} placeholder="Bijv. bar, mm, Nm" />
                                </div>
                            )}
                            ...
                            {fieldData.type === 'select' && (
                                <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700">
                                    ...
                                    <div className="flex gap-2">
                                        {bulkMode ? (
                                            <textarea 
                                                className="w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-sm" 
                                                rows={4}
                                            ...
                                            <input 
                                                type="text" 
                                                className="flex-1 p-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-sm" 
                                                value={optionInput}
                                                onChange={e => setOptionInput(e.target.value)}
                                        ...
                                        <button onClick={addOption} className="bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 transition-colors h-fit">
                            ...
                            {fieldData.type !== 'header' && (
                                <div className="grid grid-cols-2 gap-4">
                                    <label className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900/50 cursor-pointer">
                        ...
                        <div className="p-6 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0">
                            <button onClick={saveField} className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg">
                                Veld Opslaan
                            </button>
                        </div>
                    </div>
