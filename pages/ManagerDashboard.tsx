  ...
  return (
    <div className="space-y-8 pb-10">
        <div>
            ...
            <p className="text-slate-500 dark:text-slate-400">High-level overzicht van de technische dienst.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="text-3xl font-bold text-slate-800 dark:text-white">{totalAssets}</div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="text-3xl font-bold text-slate-800 dark:text-white">
                    {activeTickets.length} <span className="text-sm font-normal text-slate-400">({criticalTickets.length} kritiek)</span>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="text-3xl font-bold text-slate-800 dark:text-white">€ {totalCostYTD.toFixed(0)}</div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="text-3xl font-bold text-slate-800 dark:text-white">€ {inventoryValue.toFixed(0)}</div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* Status Pie (Links) */}
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                <h3 className="text-lg font-bold mb-6 text-slate-800 dark:text-white">Machine Status Verdeling</h3>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-red-200 dark:border-red-900/50 shadow-sm relative overflow-hidden">
                    ...
                    {criticalTickets.length > 0 ? (
                        criticalTickets.map(t => (
                            <Link 
                                ...
                                key={t.id} 
                                className="block group p-4 bg-red-50 hover:bg-red-100 dark:bg-red-900/10 dark:hover:bg-red-900/20 border border-red-100 dark:border-red-900/30 rounded-lg transition-all"
                            >
                ...
                {otherTickets.length > 0 ? (
                    otherTickets.map(t => (
                        <Link 
                            to={`/machine/${t.machineId}`}
                            key={t.id} 
                            className="block group p-4 bg-slate-50 hover:bg-slate-100 dark:bg-slate-700/30 dark:hover:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-lg transition-all"
                        >
                            <div className="flex items-start justify-between">
                                <div>
