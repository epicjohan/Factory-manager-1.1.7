                        ...
                        {article.operations.sort((a, b) => a.order - b.order).map((op, idx, allSorted) => (
                            <tr key={op.id} className="hover:bg-slate-50 dark:hover:bg-blue-900/5 transition-colors group overflow-visible">
                                ...
                                <td className="px-8 py-5 text-right">
                                    {!isLocked && (
                                        <button onClick={() => onDeleteOperation(op.id)} className="p-2 text-slate-400 hover:text-red-500 transition-all bg-slate-50 dark:bg-slate-700 rounded-2xl"><Trash2 size={16} /></button>
                                    )}
                                </td>
                            </tr>
                        ))}
