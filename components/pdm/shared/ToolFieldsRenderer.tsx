                    ...
                    return (
                        <div key={field.key} className={field.type === 'textarea' ? 'col-span-12 space-y-1' : spanClass + ' space-y-1'}>
                            ...
                            <div className="relative">
                                {field.type === 'textarea' ? (
                                    <textarea disabled={disabled || isReplaced} rows={2} className={`${inputBaseClass} ${highlightClass}`} value={val} onChange={e => onDynamicChange(field.key, e.target.value)} />
                                ) : (
                                    <input
                                        disabled={disabled || isReplaced}
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
            ...
            <div className="space-y-1">
                ...
                <button disabled={disabled || isReplaced} type="button" onClick={() => onLegacyChange('internalCooling', !tool.internalCooling)} className={`w-full p-3 rounded-[2rem] border-2 font-black text-[10px] uppercase transition-all ${tool.internalCooling ? 'bg-blue-50 border-blue-500 text-blue-600 shadow-inner' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600 text-slate-400'} disabled:opacity-60`}>
