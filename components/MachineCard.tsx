...
export const MachineCard: React.FC<MachineCardProps> = ({ machine }) => {
  ...
  return (
    <Link to={`/machine/${machine.id}`} className="block group">
      <div className={`bg-white dark:bg-slate-800 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border-2 transform hover:-translate-y-1 relative ${isGlowing ? 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.5)] scale-[1.02]' : 'border-transparent dark:border-slate-700'}`}>
        ...
        <div className="relative h-48 overflow-hidden bg-slate-100 dark:bg-slate-900">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-5">
            <h3 className="text-white font-black text-xl truncate tracking-tight" title={machine.name}>{machine.name}</h3>
            <p className="text-blue-400 text-xs font-black font-mono tracking-widest uppercase mt-0.5">{machine.machineNumber}</p>
          </div>
        </div>

        <div className="p-5 flex items-center justify-between text-slate-600 dark:text-slate-400">
