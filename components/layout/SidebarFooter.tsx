    ...
    return (
        <div className="p-4 border-t border-slate-700 bg-slate-900/50 space-y-3">
           <div className={`flex items-center ${isCollapsed ? 'justify-center flex-col gap-4' : 'justify-between'}`}>
            <button onClick={toggleTheme} className="p-2 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition-colors shrink-0">
                {theme === 'dark' ? <Sun size={20} /> : <Moon size={20} />}
            </button>
            <button onClick={onShowNotifications} className="p-2 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition-colors relative shrink-0">
                <Bell size={20} />
                {unreadNotifications > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full border border-slate-900 animate-pulse"></span>}
            </button>
            <button onClick={logout} className={`flex items-center gap-2 p-2 text-red-400 hover:bg-slate-800 hover:text-red-300 rounded-lg transition-colors shrink-0`}><LogOut size={20} /></button>
           </div>
           
           {!isCollapsed && (
             <div className="mt-4 flex flex-col gap-2">
                 <div className="flex items-center gap-3 px-3 py-3 text-slate-300 bg-slate-800/50 rounded-xl overflow-hidden">
                    <UserCircle size={32} className="shrink-0" />
                    <div className="flex flex-col min-w-0">
                        <span className="text-sm font-medium truncate" title={user?.name}>{user?.name}</span>
                        <span className="text-[10px] text-slate-500 truncate uppercase tracking-widest">{user?.role}</span>
                    </div>
                </div>
                <div className="flex gap-1.5">
                    <div className={`flex-1 flex items-center justify-between px-3 py-2 rounded-lg text-[10px] font-mono border transition-all duration-700 ${getStatusClasses()}`}>
                    ...
                    {outboxCount > 0 && (
                        <button onClick={onShowOutbox} className="bg-orange-500/20 text-orange-500 border border-orange-500/30 rounded-lg px-2 flex items-center gap-1.5 animate-pulse hover:bg-orange-500/30 transition-colors shrink-0" title="Items in wachtrij">
                            <CloudUpload size={14} />
                            <span className="text-[10px] font-black">{outboxCount}</span>
                        </button>
                    )}
                </div>
                <div className="px-3 pt-2 border-t border-slate-800 flex flex-col items-center gap-0.5">
                    <div className="text-[9px] text-slate-500 font-bold uppercase tracking-widest leading-none">v{APP_INFO.VERSION}</div>
                    <div className="text-[7px] text-slate-600 font-mono tracking-tighter uppercase leading-none opacity-60">Build: {APP_INFO.BUILD}</div>
                </div>
            </div>
           )}
        </div>
