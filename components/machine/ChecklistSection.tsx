  ...
  return (
    <div className="space-y-6">
       <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 border-b border-slate-100 dark:border-slate-700 pb-4">
              <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2"> <ClipboardList size={20} className="text-purple-500" /> Onderhouds Checklist </h3>
              <div className="flex bg-slate-100 dark:bg-slate-900 rounded-lg p-1 border border-slate-200 dark:border-slate-700">
                  <button onClick={() => setView('EXECUTE')} className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${view === 'EXECUTE' ? 'bg-white dark:bg-slate-800 text-purple-600 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-200'}`}> <CheckSquare size={14} /> Uitvoeren </button>
                  <button onClick={() => setView('HISTORY')} className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${view === 'HISTORY' ? 'bg-white dark:bg-slate-800 text-purple-600 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}> <History size={14} /> Historie </button>
              </div>
          ...
          {view === 'EXECUTE' && (
              <>
                  {showConfig && (
                      <div className="bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-800 rounded-lg p-4 mb-6 animate-in slide-in-from-top-2">
                          <h4 className="text-sm font-bold text-purple-800 dark:text-purple-300 mb-3 flex items-center gap-2"> <Plus size={16} /> Item Toevoegen </h4>
                          <form onSubmit={handleAddItem} className="space-y-4">
                              <div className="flex flex-col md:flex-row gap-3 items-end">
                                  <div className="flex-1 w-full">
                                      <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Omschrijving van de check</label>
                                      <input required type="text" placeholder="Bijv. Oliepeil controleren" className="w-full p-2 rounded border border-purple-200 dark:border-slate-600 bg-white dark:bg-slate-800 dark:text-white text-sm" value={newDesc} onChange={e => setNewDesc(e.target.value)} />
                                  </div>
                                  <div className="w-full md:w-48">
                                      <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Interval</label>
                                      <select className="w-full p-2 rounded border border-purple-200 dark:border-slate-600 bg-white dark:bg-slate-800 dark:text-white text-sm" value={newInterval} onChange={e => setNewInterval(e.target.value as ChecklistInterval)}>
                                  ...
                                  <button type="submit" className="w-full md:w-auto px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded text-sm transition-colors">Toevoegen</button>
                              </div>
                              {newInterval === ChecklistInterval.AANGEPAST && (
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in slide-in-from-top-1">
                                      <div>
                                          <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Label (Optioneel, bijv. Elke 100u)</label>
                                          <input type="text" placeholder="Laat leeg voor standaard weergave" className="w-full p-2 rounded border border-purple-200 dark:border-slate-600 bg-white dark:bg-slate-800 dark:text-white text-sm font-bold" value={newCustomText} onChange={e => setNewCustomText(e.target.value)} />
                                      </div>
                                      <div>
                                          <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Actief na (Aantal dagen)</label>
                                          <input required type="number" min="1" className="w-full p-2 rounded border border-purple-200 dark:border-slate-600 bg-white dark:bg-slate-800 dark:text-white text-sm font-bold" value={newCustomDays} onChange={e => setNewCustomDays(parseInt(e.target.value))} />
                                      </div>
                                  </div>
                              )}
                          </form>
                  ...
                  <div className="space-y-3">
                      {machine.checklist && machine.checklist.length > 0 ? (
                          machine.checklist.map(item => {
                              ...
                              return (
                                  <div key={item.id} className={`flex items-center justify-between p-4 rounded-lg border transition-all ${done ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-slate-50 dark:bg-slate-700/30 border-slate-100 dark:border-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
          ...
          {view === 'HISTORY' && (
              <div className="space-y-4">
                  <div className="flex flex-col md:flex-row gap-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                      <div className="flex-1">
                          <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 flex items-center gap-1"><Calendar size={10} /> Datum</label>
                          <input type="date" className="w-full p-2 rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 dark:text-white text-sm" value={historyDate} onChange={e => setHistoryDate(e.target.value)} />
                      </div>
                      <div className="flex-1">
                          <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 flex items-center gap-1"><Filter size={10} /> Actie Punt</label>
                          <select className="w-full p-2 rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 dark:text-white text-sm" value={historyFilterItem} onChange={e => setHistoryFilterItem(e.target.value)}>
                  ...
                  <div className="space-y-2">
                      {filteredHistory.length > 0 ? (
                          filteredHistory.map(log => {
                              ...
                              return (
                                  <div key={log.id} className="flex items-center justify-between p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg">
