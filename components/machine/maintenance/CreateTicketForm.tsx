...
export const CreateTicketForm: React.FC<CreateTicketFormProps> = ({ machineId, onClose }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-4 rounded-[2rem] border border-slate-200 dark:border-slate-700 animate-in slide-in-from-top-2 mb-6"> 
            ...
            <form onSubmit={handleSubmit} className="space-y-4"> 
                <div>
                    ...
                    <input required type="text" className="w-full p-2.5 rounded-2xl border border-slate-300 dark:border-slate-600 bg-transparent dark:text-white outline-none focus:border-blue-500 font-bold" value={title} onChange={e => setTitle(e.target.value)} placeholder="Bijv: Lekkage bij spindel..." />
                </div> 
                <div>
                    ...
                    <textarea className="w-full p-2.5 rounded-2xl border border-slate-300 dark:border-slate-600 bg-transparent dark:text-white outline-none focus:border-blue-500 text-sm" value={desc} onChange={e => setDesc(e.target.value)} placeholder="Omschrijf de situatie zo nauwkeurig mogelijk..." rows={4} />
                </div> 
                <div>
                ...
                <div className="flex gap-3 pt-2">
                    <button type="button" onClick={onClose} className="flex-1 py-3 text-slate-500 text-xs font-black uppercase tracking-widest hover:bg-slate-100 dark:hover:bg-slate-700 rounded-[2rem] transition-colors">Annuleren</button>
                    <button type="submit" className="flex-2 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-[2rem] font-black text-xs uppercase tracking-widest shadow-lg shadow-blue-500/20 px-8">Melding Opslaan</button>
                </div> 
            </form> 
        </div>
    );
