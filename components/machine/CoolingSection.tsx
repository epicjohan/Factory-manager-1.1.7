...
export const CoolingSection: React.FC<CoolingSectionProps> = ({ machine }) => {
    ...
    return (
        <div className="space-y-6">
            {/* Sub-navigatie */}
            <div className="flex bg-slate-100 dark:bg-slate-900 p-1.5 rounded-[2rem] border border-slate-200 dark:border-slate-700 w-fit">
                <button onClick={() => setActiveSubTab('REGISTRATION')} className={`px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeSubTab === 'REGISTRATION' ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 hover:text-slate-700'}`}>
                    <PlusCircle size={16} /> Registratie
                </button>
                <button onClick={() => setActiveSubTab('HISTORY')} className={`px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeSubTab === 'HISTORY' ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 hover:text-slate-700'}`}>
                    <History size={16} /> Historie
                </button>
                <button onClick={() => setActiveSubTab('TREND')} className={`px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeSubTab === 'TREND' ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 hover:text-slate-700'}`}>
                    <Activity size={16} /> Trend
                </button>
            </div>

            {activeSubTab === 'REGISTRATION' && !machine.isArchived && (
                <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm animate-in fade-in duration-300">
                    <h3 className="text-xl font-black text-slate-900 dark:text-white mb-8 uppercase tracking-tighter italic flex items-center gap-3 border-b border-slate-100 dark:border-slate-700 pb-4">
                    ...
                    <form onSubmit={handleAddLog} className="space-y-8">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1 text-left">Aanvullende Opmerkingen</label>
                                <textarea
                                    rows={2}
                                    className="w-full p-4 rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 dark:text-white outline-none focus:border-blue-500 text-sm font-medium"
                            ...
                            <div key={log.id} className={`p-5 rounded-[2rem] border transition-all shadow-sm flex flex-col gap-4 ${(log as any).isPending ? 'bg-orange-50/50 dark:bg-orange-900/10 border-orange-200' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}>
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-5">
                                        <div className={`p-4 rounded-2xl shrink-0 shadow-inner ${log.type === 'MEASUREMENT' ? 'bg-blue-50 text-blue-600' :
