...
export const MistSection: React.FC<MistSectionProps> = ({ machine }) => {
    ...
    const FilterStageCard = ({ stage, title, desc, icon: Icon, color }: any) => {
        ...
        return (
            <div className={`relative p-8 rounded-[2rem] border-2 transition-all group ${isOld ? 'border-orange-200 bg-orange-50 dark:border-orange-900/50 dark:bg-orange-900/10' : 'border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 shadow-sm'}`}>
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4">
                        <div className={`p-4 rounded-2xl ${color} text-white shadow-sm flex items-center justify-center`}>
                            <Icon size={24} />
                        </div>
                        <div>
                            <div className="flex items-center gap-2">
                                <h4 className="font-black text-slate-800 dark:text-white uppercase tracking-tighter italic text-lg">{title}</h4>
                                {isPending && <CloudCog size={16} className="text-orange-500 animate-spin-slow" />}
                            </div>
                ...
                <div className="mt-4 flex justify-between items-center">
                    ...
                    {!machine.isArchived && (
                        <button
                            ...
                            className="text-xs bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 px-5 py-2.5 rounded-full font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 transition-colors shadow-sm"
                        >
                            Vervangen
                        </button>
                    )}
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {!machine.isArchived && (
                    <div id="mist-form" className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm h-fit">
                        <h3 className="font-black text-slate-800 dark:text-white mb-6 flex items-center gap-3 uppercase tracking-tighter italic text-xl border-b border-slate-100 dark:border-slate-700 pb-4">
                            <Wind size={24} className="text-teal-500" /> Filter Wissel Registreren
                        </h3>
                        <form onSubmit={handleAddMistLog} className="space-y-4">
                            ...
                            <button type="submit" className="w-full py-4 mt-4 bg-teal-600 hover:bg-teal-700 text-white rounded-2xl font-black text-sm shadow-xl shadow-teal-500/20 transition-all active:scale-95 uppercase tracking-widest"> Registreren </button>
                        </form>
                    </div>
                )}

                <div className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm h-fit text-left">
                    <h4 className="font-black text-slate-800 dark:text-white mb-6 text-sm uppercase flex items-center justify-between tracking-widest border-b border-slate-100 dark:border-slate-700 pb-4">
                        <span>Recente Wissels</span>
                        <span className="text-xs bg-slate-100 dark:bg-slate-700 px-3 py-1 rounded-full font-mono">{augmentedLogs.length}</span>
                    </h4>
                    <div className="space-y-4 max-h-[400px] overflow-y-auto no-scrollbar">
                        {augmentedLogs.map(log => (
                            <div key={log.id} className={`p-5 rounded-2xl border transition-all ${(log as any).isPending ? 'bg-orange-50/50 dark:bg-orange-900/10 dark:border-orange-800' : 'bg-slate-50 dark:bg-slate-900/30 border-slate-100 dark:border-slate-700'}`}>
                                <div className="flex justify-between items-start gap-3">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="font-black text-slate-800 dark:text-white flex items-center gap-2 uppercase tracking-tight italic text-lg">
                                            {log.stage}
