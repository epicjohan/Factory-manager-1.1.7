...
export const CallSection: React.FC<CallSectionProps> = ({ machine }) => {
  ...
  const ActionButton: React.FC<{ type: SupportType, icon: any, label: string, colorClass: string }> = ({ type, icon: Icon, label, colorClass }) => (
      <button 
          ...
          className={`flex flex-col items-center justify-center p-4 rounded-3xl border-2 transition-all hover:scale-105 active:scale-95 shadow-sm group ${machine.isArchived ? 'opacity-40 cursor-not-allowed grayscale' : `bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:border-${colorClass}-500 dark:hover:border-${colorClass}-500`}`}
      >
          <div className={`p-3 rounded-2xl mb-3 transition-colors ${machine.isArchived ? 'bg-slate-100 dark:bg-slate-700 text-slate-400' : `bg-${colorClass}-50 dark:bg-${colorClass}-900/20 text-${colorClass}-500 group-hover:bg-${colorClass}-600 group-hover:text-white shadow-inner`}`}>
              <Icon size={24} />
          </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300 text-left">
      <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-3 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex bg-slate-100 dark:bg-slate-900 rounded-xl p-1 border border-slate-200 dark:border-slate-700">
              <button 
                  ...
                  className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${view === 'EXECUTE' ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-md ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-200'}`}
              >
                  <Megaphone size={14} /> Oproep
              </button>
              <button 
                  onClick={() => setView('HISTORY')} 
                  className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${view === 'HISTORY' ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-md ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
              >
                  <History size={14} /> Historie
              </button>
          </div>
          <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-blue-50 dark:bg-blue-900/20 rounded-full text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">
      ...
      <div className="flex items-start gap-4 p-5 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-800 shadow-inner">
          <div className="p-2 bg-blue-600 text-white rounded-xl shadow-lg shrink-0">
              <Megaphone size={16} />
          </div>
          <div>
              <h4 className="text-[10px] font-black text-blue-700 dark:text-blue-300 uppercase tracking-[0.2em] mb-1">Informatie</h4>
