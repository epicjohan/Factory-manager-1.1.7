                ...
                <form onSubmit={handleFormSubmit} className="space-y-6">
                    {activeType === SupportType.MATERIAL && (
                        <>
                            ...
                            <div className="space-y-1"><label className="block text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest">Gewenste Tijd</label><input type="time" className="w-full p-3 rounded-[2rem] border-2 border-slate-200 dark:bg-slate-700 dark:border-slate-600 dark:text-white text-sm font-bold focus:border-blue-500 outline-none transition-colors" value={materialDesiredTime} onChange={e => setMaterialDesiredTime(e.target.value)} /></div>
                    ...
                    {activeType === SupportType.SWARF && (
                        <>
                            <div className="space-y-1"><label className="block text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest">Materiaal (Inhoud)</label><select required className="w-full p-3 rounded-[2rem] border-2 border-slate-200 dark:bg-slate-700 dark:border-slate-600 dark:text-white text-sm font-bold outline-none focus:border-blue-500 transition-colors" value={swarfMaterial} onChange={e => setSwarfMaterial(e.target.value)}><option value="">-- Kies Materiaal --</option><option value="Staal (Algemeen)">Staal (Algemeen)</option><option value="RVS 304/316">RVS 304/316</option><option value="Aluminium">Aluminium</option><option value="Messing">Messing</option><option value="Kunststof">Kunststof</option><option value="Anders">Overig / Anders...</option></select></div>
                            {swarfMaterial === 'Anders' && (<div className="animate-in slide-in-from-top-2 duration-200"><label className="block text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest">Specificeer Materiaal</label><input required type="text" className="w-full p-3 rounded-[2rem] border-2 border-slate-200 dark:bg-slate-700 dark:border-slate-600 dark:text-white text-sm font-bold focus:border-blue-500 outline-none transition-colors" value={customSwarfMaterial} onChange={e => setCustomSwarfMaterial(e.target.value)} placeholder="Bijv. Inconel 718" /></div>)}
                            <div className="animate-in slide-in-from-top-2 space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        type="text" 
                                        className="w-full pl-10 p-3 rounded-[2rem] border-2 border-slate-200 dark:bg-slate-700 dark:border-slate-600 dark:text-white text-sm font-bold outline-none focus:border-blue-500 transition-colors" 
                            ...
                            <div className="space-y-1"><label className="block text-[10px] font-black text-slate-500 uppercase mb-2 tracking-widest">Urgentie</label><div className="flex gap-2"><button type="button" onClick={() => setSwarfUrgency('NORMAL')} className={`flex-1 py-3 rounded-[2rem] border-2 font-black uppercase text-[10px] tracking-widest transition-all ${swarfUrgency === 'NORMAL' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white dark:bg-slate-700 text-slate-400 border-slate-200'}`}>Normaal</button><button type="button" onClick={() => setSwarfUrgency('HIGH')} className={`flex-1 py-3 rounded-[2rem] border-2 font-black uppercase text-[10px] tracking-widest transition-all ${swarfUrgency === 'HIGH' ? 'bg-red-50 border-red-500 text-red-700' : 'bg-white dark:bg-slate-700 text-slate-400 border-slate-200'}`}>SPOED</button></div></div>
                        </>
                    )}

                    {activeType === SupportType.COOLANT && (
                        <>
                            <p className="text-xs text-slate-500 font-medium leading-relaxed bg-blue-50 dark:bg-blue-900/10 p-4 rounded-[2rem] border border-blue-100 dark:border-blue-800 shadow-inner">Vraag om bijvulling van leibaan olie of smeermiddel voor de machine-ondersteuning.</p>
                            <div className="animate-in slide-in-from-top-2 space-y-1">
                                <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest">Wanneer nodig?</label>
                                <div className="relative">
                                    <Clock className="absolute left-3 top-3.5 text-slate-400" size={16} />
                            ...
                            <div className="space-y-1"><label className="block text-[10px] font-black text-slate-500 uppercase mb-2 tracking-widest">Urgentie</label><div className="flex gap-2"><button type="button" onClick={() => setOilUrgency('NORMAL')} className={`flex-1 py-3 rounded-[2rem] border-2 font-black uppercase text-[10px] tracking-widest transition-all ${oilUrgency === 'NORMAL' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white dark:bg-slate-700 text-slate-400 border-slate-200'}`}>Normaal</button><button type="button" onClick={() => setOilUrgency('HIGH')} className={`flex-1 py-3 rounded-[2rem] border-2 font-black uppercase text-[10px] tracking-widest transition-all ${oilUrgency === 'HIGH' ? 'bg-red-50 border-red-500 text-red-700' : 'bg-white dark:bg-slate-700 text-slate-400 border-slate-200'}`}>SPOED</button></div></div>
                        </>
                    )}

                    {(activeType === SupportType.EMPTY_BIN || activeType === SupportType.BIN_EXCHANGE) && (
                        <>
                            <p className="text-xs text-slate-500 font-medium leading-relaxed bg-teal-50 dark:bg-teal-900/10 p-4 rounded-[2rem] border border-teal-100 dark:border-teal-800 shadow-inner">
                            ...
                            <div className="space-y-1"><label className="block text-[10px] font-black text-slate-500 uppercase mb-2 tracking-widest">Urgentie</label><div className="flex gap-2"><button type="button" onClick={() => setBinUrgency('NORMAL')} className={`flex-1 py-3 rounded-[2rem] border-2 font-black uppercase text-[10px] tracking-widest transition-all ${binUrgency === 'NORMAL' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white dark:bg-slate-700 text-slate-400 border-slate-200'}`}>Normaal</button><button type="button" onClick={() => setBinUrgency('HIGH')} className={`flex-1 py-3 rounded-[2rem] border-2 font-black uppercase text-[10px] tracking-widest transition-all ${binUrgency === 'HIGH' ? 'bg-red-50 border-red-500 text-red-700' : 'bg-white dark:bg-slate-700 text-slate-400 border-slate-200'}`}>SPOED</button></div></div>
                        </>
                    )}
